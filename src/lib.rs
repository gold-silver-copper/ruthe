...
mod tests {
    ...
    fn test_gc_reuse_freed_cells() {
        ...
        // v6, v7, v8 should have reused the slots from v2, v3, v4
        // (the freed cells are reused, so v2/v3/v4 indices now contain new values)
        let reused_indices = [v6, v7, v8];
        let freed_indices = [v2, v3, v4];

        // Each freed index should now be reused by one of the new allocations
        for freed_idx in freed_indices.iter() {
            assert!(
                reused_indices.contains(freed_idx),
                "Freed index {} should have been reused",
                freed_idx
            );
        }
    }
    ...
    fn test_gc_preserves_all_rooted() {
        ...
        for i in 0..10 {
            roots[i] = arena.alloc(LispValue::Number(i as i32)).unwrap();
        }
    ...
    #[test]
    fn test_gc_stress_alternating() {
        let arena: Arena<10> = Arena::new();

        // Repeatedly allocate and collect to stress test the free list
        for round in 0..5 {
            let v1 = arena.alloc(LispValue::Number(round)).unwrap();
            arena.alloc(LispValue::Number(round + 100)).unwrap();
            arena.alloc(LispValue::Number(round + 200)).unwrap();
    ...
    fn test_gc_complex_graph() {
        let arena: Arena<30> = Arena::new();

        // Build a complex graph: diamond shape
        //       root
        //      /    \
        //    left   right
        //      \    /
        ...
        let bottom_val = arena.alloc(LispValue::Number(100)).unwrap();
        let left = arena
            .alloc(LispValue::Cons(bottom_val, bottom_val))
            .unwrap();
        let right = arena
            .alloc(LispValue::Cons(bottom_val, bottom_val))
            .unwrap();
        let root = arena.alloc(LispValue::Cons(left, right)).unwrap();

        // Add garbage
        for i in 0..10 {
            arena.alloc(LispValue::Number(i)).unwrap();
        }
        ...
        // Diamond should be preserved (4 nodes), garbage collected
        assert_eq!(arena.used(), 4);
        assert!(arena.is_allocated(root));
        assert!(arena.is_allocated(left));
        assert!(arena.is_allocated(right));
        assert!(arena.is_allocated(bottom_val));
    }
}
