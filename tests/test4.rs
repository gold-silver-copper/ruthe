...
mod builtin_tests {
    ...
    // Helper function to evaluate and get a number result
    fn eval_to_number(
        arena: &Arena<DEFAULT_ARENA_SIZE>,
        input: &str,
        env: &Ref<DEFAULT_ARENA_SIZE>,
    ) -> Result<i64, ErrorCode> {
        let result = eval_string(arena, input, env)?;
        match arena.get(result.inner) {
            Some(Value::Number(n)) => Ok(n),
            _ => Err(ErrorCode::TypeError),
        }
    }

    // Helper function to evaluate and get a boolean result
    fn eval_to_bool(
        arena: &Arena<DEFAULT_ARENA_SIZE>,
        input: &str,
        env: &Ref<DEFAULT_ARENA_SIZE>,
        ...
        match arena.get(result.inner) {
            ...
            Some(Value::Nil) => Ok(false),
            _ => Ok(true), // In Scheme, everything except #f is truthy
        }
    }
    ...
    fn test_not_basic() {
        ...
        assert_eq!(eval_to_bool(&arena, "(not #f)", &env).unwrap(), true);
        assert_eq!(eval_to_bool(&arena, "(not #t)", &env).unwrap(), false);
    }

    #[test]
    fn test_not_with_values() {
        let arena = Arena::<DEFAULT_ARENA_SIZE>::new();
        let env = env_new(&arena).unwrap();

        // Everything except #f is truthy
        assert_eq!(eval_to_bool(&arena, "(not 0)", &env).unwrap(), false);
        assert_eq!(eval_to_bool(&arena, "(not 42)", &env).unwrap(), false);
        assert_eq!(eval_to_bool(&arena, "(not nil)", &env).unwrap(), false);
    }
    ...
    fn test_not_arity_error() {
        ...
        assert!(eval_string(&arena, "(not)", &env).is_err());
        assert!(eval_string(&arena, "(not #t #f)", &env).is_err());
    }
    ...
    fn test_number_predicate() {
        ...
        assert_eq!(eval_to_bool(&arena, "(number? 42)", &env).unwrap(), true);
        assert_eq!(eval_to_bool(&arena, "(number? 0)", &env).unwrap(), true);
        assert_eq!(eval_to_bool(&arena, "(number? -100)", &env).unwrap(), true);
        assert_eq!(eval_to_bool(&arena, "(number? #t)", &env).unwrap(), false);
        assert_eq!(eval_to_bool(&arena, "(number? nil)", &env).unwrap(), false);
        assert_eq!(
            eval_to_bool(&arena, "(number? (quote hello))", &env).unwrap(),
            false
        );
    }
    ...
    fn test_pair_predicate() {
        ...
        assert_eq!(
            eval_to_bool(&arena, "(pair? (cons 1 2))", &env).unwrap(),
            true
        );
        assert_eq!(
            eval_to_bool(&arena, "(pair? (list 1 2 3))", &env).unwrap(),
            true
        );
        assert_eq!(eval_to_bool(&arena, "(pair? nil)", &env).unwrap(), false);
        assert_eq!(eval_to_bool(&arena, "(pair? 42)", &env).unwrap(), false);
        assert_eq!(eval_to_bool(&arena, "(pair? #t)", &env).unwrap(), false);
    }
    ...
    fn test_symbol_predicate() {
        ...
        assert_eq!(
            eval_to_bool(&arena, "(symbol? (quote hello))", &env).unwrap(),
            true
        );
        assert_eq!(
            eval_to_bool(&arena, "(symbol? (quote x))", &env).unwrap(),
            true
        );
        assert_eq!(eval_to_bool(&arena, "(symbol? 42)", &env).unwrap(), false);
        assert_eq!(eval_to_bool(&arena, "(symbol? #t)", &env).unwrap(), false);
        assert_eq!(
            eval_to_bool(&arena, "(symbol? (cons 1 2))", &env).unwrap(),
            false
        );
    }
    ...
    fn test_procedure_predicate() {
        ...
        assert_eq!(eval_to_bool(&arena, "(procedure? +)", &env).unwrap(), true);
        assert_eq!(
            eval_to_bool(&arena, "(procedure? cons)", &env).unwrap(),
            true
        );

        eval_string(&arena, "(define f (lambda (x) x))", &env).unwrap();
        assert_eq!(eval_to_bool(&arena, "(procedure? f)", &env).unwrap(), true);

        assert_eq!(
            eval_to_bool(&arena, "(procedure? 42)", &env).unwrap(),
            false
        );
        assert_eq!(
            eval_to_bool(&arena, "(procedure? #t)", &env).unwrap(),
            false
        );
        assert_eq!(
            eval_to_bool(&arena, "(procedure? (quote hello))", &env).unwrap(),
            false
        );
    }
    ...
    fn test_eq_predicate() {
        ...
        // eq? tests identity (same object in memory)
        eval_string(&arena, "(define x (cons 1 2))", &env).unwrap();
        eval_string(&arena, "(define y x)", &env).unwrap();
        eval_string(&arena, "(define z (cons 1 2))", &env).unwrap();

        assert_eq!(eval_to_bool(&arena, "(eq? x y)", &env).unwrap(), true);
        assert_eq!(eval_to_bool(&arena, "(eq? x z)", &env).unwrap(), false);
    }

    #[test]
    fn test_equal_predicate_numbers() {
        let arena = Arena::<DEFAULT_ARENA_SIZE>::new();
        let env = env_new(&arena).unwrap();

        assert_eq!(eval_to_bool(&arena, "(equal? 42 42)", &env).unwrap(), true);
        assert_eq!(eval_to_bool(&arena, "(equal? 42 43)", &env).unwrap(), false);
        assert_eq!(eval_to_bool(&arena, "(equal? 0 0)", &env).unwrap(), true);
        assert_eq!(eval_to_bool(&arena, "(equal? -5 -5)", &env).unwrap(), true);
    }

    #[test]
    fn test_equal_predicate_bools() {
        let arena = Arena::<DEFAULT_ARENA_SIZE>::new();
        let env = env_new(&arena).unwrap();

        assert_eq!(eval_to_bool(&arena, "(equal? #t #t)", &env).unwrap(), true);
        assert_eq!(eval_to_bool(&arena, "(equal? #f #f)", &env).unwrap(), true);
        assert_eq!(eval_to_bool(&arena, "(equal? #t #f)", &env).unwrap(), false);
    }

    #[test]
    fn test_equal_predicate_lists() {
        let arena = Arena::<DEFAULT_ARENA_SIZE>::new();
        let env = env_new(&arena).unwrap();

        assert_eq!(
            eval_to_bool(&arena, "(equal? (list 1 2 3) (list 1 2 3))", &env).unwrap(),
            true
        );
        assert_eq!(
            eval_to_bool(&arena, "(equal? (list 1 2) (list 1 3))", &env).unwrap(),
            false
        );
        assert_eq!(
            eval_to_bool(&arena, "(equal? nil nil)", &env).unwrap(),
            true
        );
        assert_eq!(
            eval_to_bool(&arena, "(equal? (cons 1 2) (cons 1 2))", &env).unwrap(),
            true
        );
    }

    #[test]
    fn test_equal_predicate_symbols() {
        let arena = Arena::<DEFAULT_ARENA_SIZE>::new();
        let env = env_new(&arena).unwrap();

        assert_eq!(
            eval_to_bool(&arena, "(equal? (quote hello) (quote hello))", &env).unwrap(),
            true
        );
        assert_eq!(
            eval_to_bool(&arena, "(equal? (quote hello) (quote world))", &env).unwrap(),
            false
        );
    }
    ...
    fn test_map_basic() {
        ...
        eval_string(&arena, "(define double (lambda (x) (* x 2)))", &env).unwrap();
        let result = eval_string(&arena, "(map double (list 1 2 3))", &env).unwrap();

        // Verify result is (2 4 6)
        ...
        assert_eq!(arena.get(first.inner), Some(Value::Number(2)));
        assert_eq!(arena.get(second.inner), Some(Value::Number(4)));
        assert_eq!(arena.get(third.inner), Some(Value::Number(6)));
    }
    ...
    fn test_map_with_builtin() {
        ...
        eval_string(&arena, "(define addone (lambda (x) (+ x 1)))", &env).unwrap();
        let result = eval_string(&arena, "(map addone (list 0 1 2))", &env).unwrap();

        let first = arena.list_nth(&result, 0).unwrap();
        assert_eq!(arena.get(first.inner), Some(Value::Number(1)));
    }
    ...
    fn test_map_empty_list() {
        ...
        eval_string(&arena, "(define f (lambda (x) x))", &env).unwrap();
        let result = eval_string(&arena, "(map f nil)", &env).unwrap();

        assert!(matches!(arena.get(result.inner), Some(Value::Nil)));
    }
    ...
    fn test_filter_basic() {
        ...
        .unwrap();
        let result = eval_string(&arena, "(filter even? (list 1 2 3 4 5 6))", &env).unwrap();
    ...
    fn test_filter_all_pass() {
        ...
        eval_string(&arena, "(define always-true (lambda (x) #t))", &env).unwrap();
        let result = eval_string(&arena, "(filter always-true (list 1 2 3))", &env).unwrap();
    ...
    fn test_filter_none_pass() {
        ...
        eval_string(&arena, "(define always-false (lambda (x) #f))", &env).unwrap();
        let result = eval_string(&arena, "(filter always-false (list 1 2 3))", &env).unwrap();
    ...
    fn test_foldl_sum() {
        ...
        let result = eval_to_number(&arena, "(foldl + 0 (list 1 2 3 4))", &env).unwrap();
        assert_eq!(result, 10);
    }

    #[test]
    fn test_foldl_product() {
        let arena = Arena::<DEFAULT_ARENA_SIZE>::new();
        let env = env_new(&arena).unwrap();

        let result = eval_to_number(&arena, "(foldl * 1 (list 2 3 4))", &env).unwrap();
        assert_eq!(result, 24);
    }

    #[test]
    fn test_foldl_with_lambda() {
        let arena = Arena::<DEFAULT_ARENA_SIZE>::new();
        let env = env_new(&arena).unwrap();

        eval_string(&arena, "(define sub-fn (lambda (acc x) (- acc x)))", &env).unwrap();
        let result = eval_to_number(&arena, "(foldl sub-fn 10 (list 1 2 3))", &env).unwrap();
        // 10 - 1 = 9, 9 - 2 = 7, 7 - 3 = 4
        assert_eq!(result, 4);
    }

    #[test]
    fn test_foldl_empty_list() {
        let arena = Arena::<DEFAULT_ARENA_SIZE>::new();
        let env = env_new(&arena).unwrap();

        let result = eval_to_number(&arena, "(foldl + 42 nil)", &env).unwrap();
        assert_eq!(result, 42); // Should return the initial value
    }
    ...
    fn test_lte_basic() {
        ...
        assert_eq!(eval_to_bool(&arena, "(<= 3 5)", &env).unwrap(), true);
        assert_eq!(eval_to_bool(&arena, "(<= 5 5)", &env).unwrap(), true);
        assert_eq!(eval_to_bool(&arena, "(<= 7 5)", &env).unwrap(), false);
        assert_eq!(eval_to_bool(&arena, "(<= -10 0)", &env).unwrap(), true);
    }
    ...
    fn test_gte_basic() {
        ...
        assert_eq!(eval_to_bool(&arena, "(>= 5 3)", &env).unwrap(), true);
        assert_eq!(eval_to_bool(&arena, "(>= 5 5)", &env).unwrap(), true);
        assert_eq!(eval_to_bool(&arena, "(>= 3 5)", &env).unwrap(), false);
        assert_eq!(eval_to_bool(&arena, "(>= 0 -10)", &env).unwrap(), true);
    }

    #[test]
    fn test_lte_gte_edge_cases() {
        let arena = Arena::<DEFAULT_ARENA_SIZE>::new();
        let env = env_new(&arena).unwrap();

        assert_eq!(eval_to_bool(&arena, "(<= 0 0)", &env).unwrap(), true);
        assert_eq!(eval_to_bool(&arena, "(>= 0 0)", &env).unwrap(), true);
    }
    ...
    fn test_min_basic() {
        ...
        assert_eq!(eval_to_number(&arena, "(min 3 5)", &env).unwrap(), 3);
        assert_eq!(eval_to_number(&arena, "(min 10 2)", &env).unwrap(), 2);
        assert_eq!(eval_to_number(&arena, "(min -5 5)", &env).unwrap(), -5);
    }
    ...
    fn test_min_multiple_args() {
        ...
        assert_eq!(eval_to_number(&arena, "(min 5 3 8 1 9)", &env).unwrap(), 1);
        assert_eq!(
            eval_to_number(&arena, "(min 100 50 75 25)", &env).unwrap(),
            25
        );
    }
    ...
    fn test_min_single_arg() {
        ...
        assert_eq!(eval_to_number(&arena, "(min 42)", &env).unwrap(), 42);
    }
    ...
    fn test_max_basic() {
        ...
        assert_eq!(eval_to_number(&arena, "(max 3 5)", &env).unwrap(), 5);
        assert_eq!(eval_to_number(&arena, "(max 10 2)", &env).unwrap(), 10);
        assert_eq!(eval_to_number(&arena, "(max -5 5)", &env).unwrap(), 5);
    }
    ...
    fn test_max_multiple_args() {
        ...
        assert_eq!(eval_to_number(&arena, "(max 5 3 8 1 9)", &env).unwrap(), 9);
        assert_eq!(
            eval_to_number(&arena, "(max 100 50 75 25)", &env).unwrap(),
            100
        );
    }
    ...
    fn test_modulo_basic() {
        ...
        assert_eq!(eval_to_number(&arena, "(modulo 10 3)", &env).unwrap(), 1);
        assert_eq!(eval_to_number(&arena, "(modulo 15 4)", &env).unwrap(), 3);
        assert_eq!(eval_to_number(&arena, "(modulo 8 2)", &env).unwrap(), 0);
        assert_eq!(eval_to_number(&arena, "(modulo 7 7)", &env).unwrap(), 0);
    }
    ...
    fn test_modulo_division_by_zero() {
        ...
        assert!(eval_string(&arena, "(modulo 10 0)", &env).is_err());
    }
    ...
    fn test_quotient_basic() {
        ...
        assert_eq!(eval_to_number(&arena, "(quotient 10 3)", &env).unwrap(), 3);
        assert_eq!(eval_to_number(&arena, "(quotient 15 4)", &env).unwrap(), 3);
        assert_eq!(eval_to_number(&arena, "(quotient 8 2)", &env).unwrap(), 4);
        assert_eq!(eval_to_number(&arena, "(quotient 7 7)", &env).unwrap(), 1);
    }
    ...
    fn test_quotient_negative() {
        ...
        assert_eq!(
            eval_to_number(&arena, "(quotient -10 3)", &env).unwrap(),
            -3
        );
        assert_eq!(
            eval_to_number(&arena, "(quotient 10 -3)", &env).unwrap(),
    ...
    fn test_quotient_division_by_zero() {
        ...
        assert!(eval_string(&arena, "(quotient 10 0)", &env).is_err());
    }
    ...
    fn test_abs_basic() {
        ...
        assert_eq!(eval_to_number(&arena, "(abs 5)", &env).unwrap(), 5);
        assert_eq!(eval_to_number(&arena, "(abs -5)", &env).unwrap(), 5);
        assert_eq!(eval_to_number(&arena, "(abs 0)", &env).unwrap(), 0);
        assert_eq!(eval_to_number(&arena, "(abs -100)", &env).unwrap(), 100);
    }
    ...
    fn test_zero_predicate() {
        ...
        assert_eq!(eval_to_bool(&arena, "(zero? 0)", &env).unwrap(), true);
        assert_eq!(eval_to_bool(&arena, "(zero? 1)", &env).unwrap(), false);
        assert_eq!(eval_to_bool(&arena, "(zero? -1)", &env).unwrap(), false);
        assert_eq!(eval_to_bool(&arena, "(zero? 100)", &env).unwrap(), false);
    }
    ...
    fn test_positive_predicate() {
        ...
        assert_eq!(eval_to_bool(&arena, "(positive? 1)", &env).unwrap(), true);
        assert_eq!(eval_to_bool(&arena, "(positive? 100)", &env).unwrap(), true);
        assert_eq!(eval_to_bool(&arena, "(positive? 0)", &env).unwrap(), false);
        assert_eq!(eval_to_bool(&arena, "(positive? -1)", &env).unwrap(), false);
        assert_eq!(
            eval_to_bool(&arena, "(positive? -100)", &env).unwrap(),
            false
        );
    }
    ...
    fn test_negative_predicate() {
        ...
        assert_eq!(eval_to_bool(&arena, "(negative? -1)", &env).unwrap(), true);
        assert_eq!(
            eval_to_bool(&arena, "(negative? -100)", &env).unwrap(),
            true
        );
        assert_eq!(eval_to_bool(&arena, "(negative? 0)", &env).unwrap(), false);
        assert_eq!(eval_to_bool(&arena, "(negative? 1)", &env).unwrap(), false);
        assert_eq!(
            eval_to_bool(&arena, "(negative? 100)", &env).unwrap(),
            false
        );
    ...
    fn test_display() {
        ...
        // display should return nil
        let result = eval_string(&arena, "(display 42)", &env).unwrap();
        assert!(matches!(arena.get(result.inner), Some(Value::Nil)));
    }
    ...
    fn test_newline() {
        ...
        // newline should return nil
        let result = eval_string(&arena, "(newline)", &env).unwrap();
        assert!(matches!(arena.get(result.inner), Some(Value::Nil)));
    }
    ...
    #[test]
    fn test_filter_with_predicates() {
        let arena = Arena::<DEFAULT_ARENA_SIZE>::new();
        let env = env_new(&arena).unwrap();

        // Filter positive numbers
        let result = eval_string(&arena, "(filter positive? (list -2 -1 0 1 2))", &env).unwrap();
        assert_eq!(arena.list_len(&result), 2);
    ...
    #[test]
    fn test_map_with_abs() {
        let arena = Arena::<DEFAULT_ARENA_SIZE>::new();
        let env = env_new(&arena).unwrap();

        let result = eval_string(&arena, "(map abs (list -5 -3 2 -8))", &env).unwrap();

        let vals: Vec<i64> = (0..4)
            .map(|i| {
                let elem = arena.list_nth(&result, i).unwrap();
                match arena.get(elem.inner) {
        ...
        assert_eq!(vals, vec![5, 3, 2, 8]);
    }

    #[test]
    fn test_foldl_with_max() {
        let arena = Arena::<DEFAULT_ARENA_SIZE>::new();
        let env = env_new(&arena).unwrap();

        // Find maximum using foldl
        eval_string(
            &arena,
            "(define find-max (lambda (acc x) (max acc x)))",
            &env,
        )
        .unwrap();
        let result = eval_to_number(&arena, "(foldl find-max 0 (list 3 7 2 9 4))", &env).unwrap();
        assert_eq!(result, 9);
    }
    ...
    fn test_complex_expression() {
        ...
        eval_string(
            &arena,
            "(define positive-and-even? (lambda (x) (if (positive? x) (even? x) #f)))",
            &env,
        )
        .unwrap();
        ...
        let result = eval_string(
            &arena,
            "(filter positive-and-even? (list -4 -2 0 1 2 3 4 5 6))",
            &env,
        )
        .unwrap();
        assert_eq!(arena.list_len(&result), 3); // 2, 4, 6
    }

    #[test]
    fn test_map_quotient() {
        let arena = Arena::<DEFAULT_ARENA_SIZE>::new();
        let env = env_new(&arena).unwrap();

        eval_string(&arena, "(define halve (lambda (x) (quotient x 2)))", &env).unwrap();
        let result = eval_string(&arena, "(map halve (list 10 20 30 40))", &env).unwrap();

        let first = arena.list_nth(&result, 0).unwrap();
        let second = arena.list_nth(&result, 1).unwrap();

        assert_eq!(arena.get(first.inner), Some(Value::Number(5)));
        assert_eq!(arena.get(second.inner), Some(Value::Number(10)));
    }
    ...
    fn test_nested_predicates() {
        ...
        // Test that not and predicates work together
        assert_eq!(eval_to_bool(&arena, "(not (zero? 5))", &env).unwrap(), true);
        assert_eq!(
            eval_to_bool(&arena, "(not (positive? -3))", &env).unwrap(),
            true
        );
        assert_eq!(
            eval_to_bool(&arena, "(not (negative? 10))", &env).unwrap(),
            true
        );
    }
    ...
    fn test_comparison_chain() {
        ...
        // Test combining comparison operators
        eval_string(
            &arena,
            "(define in-range? (lambda (x) (if (>= x 0) (<= x 10) #f)))",
            &env,
        )
        .unwrap();

        assert_eq!(eval_to_bool(&arena, "(in-range? 5)", &env).unwrap(), true);
        assert_eq!(eval_to_bool(&arena, "(in-range? -1)", &env).unwrap(), false);
        assert_eq!(eval_to_bool(&arena, "(in-range? 11)", &env).unwrap(), false);
    }

    #[test]
    fn test_equal_with_map() {
        let arena = Arena::<DEFAULT_ARENA_SIZE>::new();
        let env = env_new(&arena).unwrap();

        eval_string(&arena, "(define double (lambda (x) (* x 2)))", &env).unwrap();
        eval_string(&arena, "(define list1 (map double (list 1 2 3)))", &env).unwrap();
        eval_string(&arena, "(define list2 (list 2 4 6))", &env).unwrap();

        assert_eq!(
            eval_to_bool(&arena, "(equal? list1 list2)", &env).unwrap(),
            true
        );
    }
    ...
    fn test_type_errors() {
        ...
        // These should fail because they require numbers
        assert!(eval_string(&arena, "(+ 1 #t)", &env).is_err());
        assert!(eval_string(&arena, "(modulo #f 2)", &env).is_err());
        assert!(eval_string(&arena, "(abs (quote hello))", &env).is_err());
    }
    ...
    fn test_arity_errors() {
        ...
        assert!(eval_string(&arena, "(abs)", &env).is_err());
        assert!(eval_string(&arena, "(abs 1 2)", &env).is_err());
        assert!(eval_string(&arena, "(zero?)", &env).is_err());
        assert!(eval_string(&arena, "(positive? 1 2)", &env).is_err());
        assert!(eval_string(&arena, "(modulo 10)", &env).is_err());
        assert!(eval_string(&arena, "(quotient 10 3 5)", &env).is_err());
    }

    #[test]
    fn test_map_not_callable() {
        let arena = Arena::<DEFAULT_ARENA_SIZE>::new();
        let env = env_new(&arena).unwrap();

        // Try to map with a non-procedure
        assert!(eval_string(&arena, "(map 42 (list 1 2 3))", &env).is_err());
    }
}
